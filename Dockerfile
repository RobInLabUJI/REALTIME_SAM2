...
RUN sudo apt-get update && sudo apt-get install -y \
    python3-flask ninja-build \
    && sudo rm -rf /var/lib/apt/lists/*

RUN sudo pip install --upgrade packaging

RUN sudo python3 -m pip install torch torchvision --ignore-installed

RUN git clone https://github.com/RobInLabUJI/REALTIME_SAM2 && \
    cd REALTIME_SAM2 && \
    sudo python3 -m pip install -e . -v
